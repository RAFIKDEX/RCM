...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(!preg_match('/^\d{2,6}$/',$values['ivr_num']))               $errors[]='Invalid IVR number (26 digits)';
    if(!preg_match('/^[a-zA-Z0-9_-]{2,30}$/',$values['ivr_name'])) $errors[]='Invalid IVR name (230 chars, a-z 0-9 _ -)';
    if(!preg_match('/^[1-9]$/',$values['loop_count']))              $errors[]='Loop count must be 19';
