...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($office_time_name === '') {
        ...
        try {
            $stmt = $conn_time->prepare("INSERT INTO office_times (name) VALUES (?)");
            if (!$stmt) {
                throw new Exception('Prepare failed for office_times: ' . $conn_time->error);
            }

            $stmt->bind_param("s", $office_time_name);

            if (!$stmt->execute()) {
                throw new Exception('Execute failed for office_times: ' . $stmt->error);
            }

            $office_time_id = $stmt->insert_id;
            $stmt->close();
            ...
            if (!$entry_stmt) {
                throw new Exception('Prepare failed for office_time_entries: ' . $conn_time->error);
            }
            ...
            foreach ($entries as $entry) {
                ...
                if (!$entry_stmt->execute()) {
                    throw new Exception('Execute failed for office_time_entries: ' . $entry_stmt->error);
                }
            }
            ...
            header("Location: add_office_time.php?success=1");
            exit;
        } catch (Throwable $e) {
            $conn_time->rollback();
...
if (isset($_GET['success']) && $_GET['success'] == '1') {
    $success_message = 'Office Time saved successfully.';
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
  <div class="wrap">
    <div class="top-actions">
      <div>
        <h1 class="page-title" style="margin-bottom:8px;">ADD OFFICE TIME</h1>
        <p class="page-subtitle">Create one named Office Time and add multiple time ranges inside it.</p>
      </div>
        ...
        <div class="builder-card">
          ...
          <div class="field-row">
            <div class="field">
              ...
              <input type="text" name="office_time_name" id="office_time_name" class="input" placeholder="Example: CUC">
            </div>
          </div>
    ...
    <table>
      ...
      <tbody id="entriesBody"></tbody>
          ...
          <div class="bottom-actions">
            <button type="submit" class="btn">Save Office Time</button>
