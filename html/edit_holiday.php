...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($holiday_name === '') {
        ...
        try {
            ...
            $update_stmt = $conn_time->prepare("UPDATE holidays SET name = ?, memo = ? WHERE id = ?");
            if (!$update_stmt) {
                throw new Exception('Prepare failed for holidays update: ' . $conn_time->error);
            }

            $update_stmt->bind_param("ssi", $holiday_name, $empty_memo, $id);

            if (!$update_stmt->execute()) {
                throw new Exception('Execute failed for holidays update: ' . $update_stmt->error);
            }
            $update_stmt->close();

            $delete_entries_stmt = $conn_time->prepare("DELETE FROM holiday_entries WHERE holiday_id = ?");
            if (!$delete_entries_stmt) {
                throw new Exception('Prepare failed for deleting old holiday entries: ' . $conn_time->error);
            }

            $delete_entries_stmt->bind_param("i", $id);

            if (!$delete_entries_stmt->execute()) {
                throw new Exception('Execute failed for deleting old holiday entries: ' . $delete_entries_stmt->error);
            }
            ...
            if (!$insert_entry_stmt) {
                throw new Exception('Prepare failed for holiday_entries insert: ' . $conn_time->error);
            }
            ...
            foreach ($entries as $entry) {
                ...
                if (!$insert_entry_stmt->execute()) {
                    throw new Exception('Execute failed for holiday_entries insert: ' . $insert_entry_stmt->error);
                }
            }
            ...
            header("Location: /edit_holiday.php?id=" . $id . "&success=1");
            exit;
        } catch (Throwable $e) {
            $conn_time->rollback();
...
$entry_stmt = $conn_time->prepare("
    SELECT id, years, months, month_days, time_from, time_to
...
while ($row = $entries_result->fetch_assoc()) {
    $existing_entries[] = [
        ...
        'from' => $row['time_from'] ? substr($row['time_from'], 0, 5) : '00:00',
        'to' => $row['time_to'] ? substr($row['time_to'], 0, 5) : '23:59'
    ];
}
$entry_stmt->close();
...
<body>
  <div class="wrap">
    <div class="top-actions">
      <div>
        <h1 class="page-title" style="margin-bottom:8px;">EDIT HOLIDAY</h1>
        <p class="page-subtitle">Update the Holiday and its entries.</p>
      </div>
          ...
          <div class="field-row">
            <div class="field">
              ...
              <input
                ...
                id="holiday_name"
                class="input"
                value="<?php echo htmlspecialchars($holiday['name']); ?>"
                placeholder="Example: Eid 2026"
              >
            </div>
          </div>
