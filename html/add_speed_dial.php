...
function add_speed_dial_to_context($file, $line){
  $content = file_exists($file) ? file_get_contents($file) : "";
  if ($content === false) return false;

  if (!preg_match('/\[speed-dials\]/i', $content)) {
    $content .= "\n[speed-dials]\n";
  }
  ...
  foreach ($lines as $one) {
    ...
    if (preg_match('/^\[speed-dials\]$/i', $trim)) {
      $inContext = true;
      $newLines[] = $one;
      continue;
    }

    if ($inContext && preg_match('/^\[[^]]+\]$/', $trim)) {
      $newLines[] = $line;
      $inserted = true;
      $inContext = false;
      $newLines[] = $one;
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($extension === "") {
    ...
    if ($line === "") {
      ...
      if (add_speed_dial_to_context($CFG_FILE, $line)) {
     ...
     exec("sudo /usr/sbin/asterisk -rx 'core reload' 2>&1");
        ...
        $error = "Failed to write to extensions_gui.conf";
      }
    }
  }
}
...
<div class="wrap">
  <h1 class="page-title">ADD SPEED DIAL</h1>
    ...
    <form method="post">
      <div class="field">
        ...
        <div class="check-wrap">
          ...
          <span class="pill"> Powered by RAFIK </span>
        </div>
      </div>
