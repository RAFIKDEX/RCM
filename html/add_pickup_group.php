...
function parse_existing_pickup_groups($file){
  if (!file_exists($file)) return [];
...
function next_pickup_group_number($groups){
  $max = 0;
  foreach ($groups as $g) {
    $n = (int)$g["group"];
    if ($n > $max) {
...
function append_pickup_group_section($file, $groupNumber, $name, $members){
  $block = [];
  $block[] = "";
  $block[] = "; --- PICKUP GROUP GUI: " . $name . " ---";
  $block[] = "[pickup-" . $groupNumber . "]";
  $block[] = "name=" . $name;
  $block[] = "group=" . $groupNumber;
  $block[] = "members=" . implode(",", $members);
  $block[] = "";

  $content = implode(PHP_EOL, $block);
  return file_put_contents($file, $content, FILE_APPEND | LOCK_EX) !== false;
}

function apply_group_to_extensions($file, $members, $groupNumber){
  ...
  $normalizeGroups = function($value){
    $parts = explode(',', (string)$value);
    $parts = array_map('trim', $parts);
    $parts = array_values(array_filter($parts, fn($x) => $x !== ''));
    $parts = array_values(array_unique($parts, SORT_STRING));
  ...
  $mergeGroup = function($existing, $groupNumber) use ($normalizeGroups){
    $parts = $normalizeGroups($existing);
    $g = (string)$groupNumber;
  ...
  foreach ($sections as $section) {
    ...
    $finalCall = $mergeGroup($existingCall, $groupNumber);
    $finalPickup = $mergeGroup($existingPickup, $groupNumber);
...
$existingGroups = parse_existing_pickup_groups($EXT_GUI_FILE);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($name === "") {
    ...
    if ($existsSameName) {
      ...
      $groupNumber = next_pickup_group_number($existingGroups);

      $ok1 = append_pickup_group_section($EXT_GUI_FILE, $groupNumber, $name, $selectedMembers);
      if (!$ok1) {
        $error = "Failed to update extensions_gui.conf.";
      } else {
        $ok2 = apply_group_to_extensions($PJSIP_FILE, $selectedMembers, $groupNumber);

        if (!$ok2) {
          $error = "Pickup Group was added in extensions_gui.conf, but failed to update pjsip.gui.endpoint.conf.";
        } else {
          $reload = reload_asterisk();

          if (!$reload["ok"]) {
            $error = "Pickup Group saved, but Asterisk core reload failed.";
          } else {
...
<div class="wrap">
  <h1 class="page-title">ADD PICKUP GROUP</h1>
